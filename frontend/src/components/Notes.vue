...
const notesUrl = computed(() =>
  props.nhs
    ? `/fishers/${encodeURIComponent(props.nhs)}/notes`
    : `/households/${encodeURIComponent(props.pah)}/notes`
)
...
async function loadNotes () {
  ...
  error.value = ''
  try {
    const res = await axiosSecure.get(notesUrl.value)
...
async function submitNote () {
  ...
  saveError.value = ''
  try {
    const res = await axiosSecure.post(notesUrl.value, { note: text })
